...
public class ClawControl : MonoBehaviour {
	...
	public void Atirar(){
		float x = Direcao();
		if (x < 0) {
			GameObject l = GameObject.Instantiate (shoot, atirar.transform.position, atirar.transform.rotation * Quaternion.Euler(new Vector3(0,0,270))) as GameObject;
		} else {
			GameObject l = GameObject.Instantiate (shoot, atirar.transform.position, atirar.transform.rotation * Quaternion.Euler(new Vector3(0,0,90))) as GameObject;
		}
